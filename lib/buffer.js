    ...
    allLines.forEach(function (line)
      {
       ...
       // ignore eventTypes values
        ...
        sails.log.info('Found new Jeedom device: ' + parts[0].trim());
      }
    );
  ...
  receivedValue: function(allLines) {
    ...
    allLines.forEach(function (line)
    {
      ...
      //  .then(function(devicetype) {
           return gladys.deviceState.createByIdentifier(item.id, 'jeedom', item.type, {value:item.value});
        })
        .catch(function(err) {
          sails.log.warn(err);
